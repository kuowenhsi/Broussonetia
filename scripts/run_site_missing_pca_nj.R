...
required <- c("vcfR", "ape", "ggplot2", "data.table", "ggrepel")
...
vcf_in <- "VCF/filtered/Broussonetia_RADseq.08_missing50_mac1.vcf.gz"
metadata_file <- "reports/missing50_mac1/PRJNA437223_Broussonetia_meta.missing50_mac1.csv"
out_dir <- "reports/site_missing_pca_nj"
...
needed_cols <- c("Run", "geo_loc_name_country", "geo_loc_name")
...
if (!identical(metadata$Run, samples)) {
  ...
  stop(
    "Missing metadata for VCF sample(s): ",
...
run_threshold <- function(threshold) {
  ...
  filtered_vcf <- file.path(vcf_dir, paste0("Broussonetia_RADseq.08_missing50_mac1_", tag, ".vcf.gz"))
  ...
  pca_dt <- merge(pca_dt, metadata[, .(Run, country, locality)], by = "Run", sort = FALSE)
  ...
  pca_plot <- ggplot(pca_dt, aes(PC1, PC2, color = country)) +
    ...
    labs(
      ...
      color = "Country",
      title = sprintf("Broussonetia RADseq PCA, site missing <= %d%%", threshold),
  ...
  message("Calculating NJ tree for ", tag)
  dist_obj <- pairwise_genetic_distance(dosage)
  tree <- nj(dist_obj)
  tree$tip.label <- metadata$locality[match(tree$tip.label, metadata$Run)]
  write.tree(tree, file.path(out_dir, paste0(tag, "_nj_tree.newick")))

  tip_country <- metadata$country[match(samples, metadata$Run)]
  names(tip_country) <- metadata$locality[match(samples, metadata$Run)]
  tip_colors <- country_palette[as.character(metadata$country[match(tree$tip.label, metadata$locality)])]
  tip_colors[is.na(tip_colors)] <- "grey30"

  png(file.path(out_dir, paste0(tag, "_nj_tree_location_labels.png")), width = 6000, height = 7200, res = 300)
  par(mar = c(1, 1, 3, 1))
  plot(
    tree,
    type = "phylogram",
    ...
    label.offset = 0.002,
    main = sprintf("Neighbor-joining tree, site missing <= %d%%", threshold)
  )
  legend(
    "topleft",
  ...
  pdf(file.path(out_dir, paste0(tag, "_nj_tree_location_labels.pdf")), width = 20, height = 24)
  par(mar = c(1, 1, 3, 1))
  plot(
    tree,
    type = "phylogram",
